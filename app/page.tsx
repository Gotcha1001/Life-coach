...
function IntakeFlow({ onComplete }: { onComplete: () => void }) {
  ...
  return (
    <div className="min-h-screen bg-radial from-purple-500 to-indigo-900 flex flex-col">
      <nav className="flex items-center justify-between px-6 py-5 max-w-2xl mx-auto w-full">
        <span className="font-semibold text-base tracking-tight text-foreground">
          Life Coach <span className="text-primary">AI</span>
        </span>
            ...
            >
              ...
              {currentStep.type === "textarea" && (
                <textarea
                  rows={5}
                  autoFocus
                  className="w-full rounded-xl border border-border bg-white text-black
                             p-4 text-sm resize-none focus:outline-none focus:ring-2
                             focus:ring-primary placeholder:text-muted-foreground"
                  placeholder="Type your answer here..."
                  value={data[currentStep.field] as string}
              ...
              {currentStep.type === "textarea" && (
                <p className="text-xs text-white mt-2">⌘ Enter to continue</p>
              )}
              ...
              {loading && (
                <div className="mt-6 rounded-xl bg-card border border-border p-5 text-sm text-white space-y-1.5">
                  <p className="font-medium text-white">Building your plan…</p>
                  <p>Analysing your goal and timeline.</p>
                  <p>Generating month-by-month milestones.</p>
                  <p>This takes about 15 seconds.</p>
                </div>
...
// ─── Waiting screen for the redirect-in-flight case ───────────────────────────
// Shown briefly while we push the signed-in user with an active session to
// /dashboard/plan. Includes a manual link so the page is never a dead end if
// the redirect is ever slow, blocked, or the effect hasn't fired yet.

function RedirectingToPlan() {
  const router = useRouter();

  return (
    <div className="min-h-screen flex items-center justify-center bg-radial from-purple-500 to-indigo-900">
      <div className="text-center space-y-4">
        <p className="text-sm text-white">Taking you to your plan…</p>
        <button
...
export default function HomePage() {
  ...
  // Was previously `return null;` — that left the page permanently blank
  // any time the redirect effect didn't fire (slow network, blocked
  // navigation, etc). Now we show a small waiting state with a manual
  // fallback link instead of nothing at all.
  if (hasActiveSession) return <RedirectingToPlan />;
